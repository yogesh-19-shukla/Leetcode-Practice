class Solution {
private:
    int f(int n, vector<int>& prices, int ind, int buy, vector<vector<int>>&dp)
    {
        ...
        if(buy){
           profit = max(-prices[ind] + f(n,prices,ind+1,0,dp), 0 + f(n,prices,ind+1,1,dp));
        }
        // max of either we sell and can buy again 
        //or we dont sell and move ahead but can't buy again
        else{
           profit = max(prices[ind] + f(n,prices,ind+1,1,dp), 0 + f(n,prices,ind+1,0,dp));
        }
